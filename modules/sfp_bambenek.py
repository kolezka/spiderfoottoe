...
class sfp_bambenek(SpiderFootAsyncPlugin):
    ...
    def setup(self, sfc: SpiderFoot, userOpts: dict = None) -> None:
        ...
        self.feedCache = {
            "dga_domains": self.retrieveDataFromFeed(
                "https://osint.bambenekconsulting.com/feeds/dga-feed.txt"
            ),
            "c2_ips": self.retrieveDataFromFeed(
                "https://osint.bambenekconsulting.com/feeds/c2-ipmasterlist.txt"
            ),
            "c2_domains": self.retrieveDataFromFeed(
    ...
    def parseData(self, data: str) -> list:
        ...
        for line in data.splitlines():
            ...
            # First column is typically the domain/IP
    ...
    def handleEvent(self, event: SpiderFootEvent) -> None:
        ...
        if eventName == "IP_ADDRESS":
            ...
            is_malicious = (
                self.queryFeed(eventData, "c2_ips")
                if self.opts.get("c2_feed", True)
                else False
            )
            ...
            malicious_types = []
            if self.opts.get("dga_feed", True) and self.queryFeed(
                eventData, "dga_domains"
            ):
                malicious_types.append(
                    ("BLACKLISTED_INTERNET_NAME",
                     "Bambenek Consulting [DGA Domain]")
                )
            if self.opts.get("c2_feed", True) and self.queryFeed(
                eventData, "c2_domains"
            ):
                malicious_types.append(
                    ("BLACKLISTED_INTERNET_NAME",
            ...
            if self.opts.get("dga_feed", True) and self.queryFeed(
                ...
                malicious_types.append(
                    ("BLACKLISTED_COHOST", "Bambenek Consulting [DGA Domain]")
                )
            if self.opts.get("c2_feed", True) and self.queryFeed(
                eventData, "c2_domains"
            ):
                malicious_types.append(
                    ("BLACKLISTED_COHOST", "Bambenek Consulting [C2 Domain]")
                )
